...
print(f'对比实验: {n_ships} ships × {n_tasks} tasks, 5 seeds')
...
for seed in range(N_SEEDS):
    ...
    sh = build_scene()
    for tid in sh.tasks:
        sh.tasks[tid].payload = float(np.random.choice([300, 500, 800, 1000, 1500]))
    ...
    with contextlib.redirect_stdout(io.StringIO()):
        ...
        sched.initialize(initial_routes=greedy_routes_alns)
    results['ALNS']['tm'].append(time.perf_counter() - t0)
    ms, td, en, ls = evaluate(sh, sched.routes)
